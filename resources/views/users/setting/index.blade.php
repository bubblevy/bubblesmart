...
@endsection
<div class="infoupdate" data-user-updated="@if(session()->has('updateUserBerhasil')) {{ session('updateUserBerhasil') }} @endif"></div>
    ...
    <div class="nav-align-top">
      ...
      <div class="tab-content">
        <div class="tab-pane fade @unless ($errors->has('passwordLama') || $errors->has('passwordBaru') || session()->has('passwordLamaSalah')) show active @endunless" id="navs-profil" role="tabpanel">
          ...
          <!-- Profile  -->
          <div class="card-body">
            <form id="formAccountSettings" action="/pengaturan" method="POST" enctype="multipart/form-data">
              @csrf
              <div class="d-flex align-items-start align-items-sm-center gap-4">
                <img src="@if(Storage::disk('public')->exists(auth()->user()->image)) {{ asset('storage/'. auth()->user()->image) }} @else {{ asset('assets/img/'. auth()->user()->image) }} @endif" alt="profile" class="d-block cursor-pointer rounded fotoProfile" height="100" width="100" id="uploadedPhotoProfil" data-url-img="@if(Storage::disk('public')->exists('profil-images')) {{ asset('storage/'. auth()->user()->image) }} @else {{ asset('assets/img/'. auth()->user()->image) }} @endif" />
...
<div class="modal fade" id="formModalUsersEditEmail" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form action="/pengaturan/verify" method="post" class="modalUsersEditEmail">
      @csrf
...
<div class="modal fade" id="formModalUsersSetEmail" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <form action="/pengaturan/setemail" method="post" class="modalUsersSetEmail">
      @csrf
      <div class="modal-content">
        <div class="modal-header d-flex justify-content-between">
...
@section('script')
<script src="{{ asset('assets/js/settings/usersetting.js') }}"></script>
@endsection
@endsection